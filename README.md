...
- 所有 source-mode 與 build-mode Demo 的預設主題統一為 `default`；選擇其他 theme 時會用 query string 重新載入頁面與對應 CSS，不在 runtime 切換 theme class。公開元件仍統一公布 19 組 theme metadata。第 17 至 19 組為 `mono`、`mono-red`、`mono-green`，分別使用 Metro Gray、Metro Red、Metro Green 的版面色調，並共用 `src/theme/mono/images/` 的單色 SVG；正式 build 統一輸出 icon 到 `dist/theme/mono/`。三組 Mono theme 的 sprite icon 均拆成獨立 SVG，CSS 不引用 PNG／GIF。
- `fabui.css`／`fabui.min.css` 與 Lite CSS 只內建 Default theme；其餘 18 組 `dist/theme/fabui.<theme>.css` 使用與 Default 相同的固定 component selector，必須在所有 FabUI／附加元件 CSS 之後載入。切換主題時更換這份額外 CSS 並重新載入頁面，不需要 `fg-theme-<theme>` class 或 `setTheme()`。
...
Default theme 不需要載入額外 CSS。使用其他 theme 時，在所有 FabUI 與附加元件 CSS 之後載入對應檔案；若要換 theme，請更換這個檔案並重新載入頁面：
...
| 表格版面 | 支援左右凍結欄、列號欄、欄寬調整、欄位顯示切換、Footer aggregate、交替列背景與 Grid 全螢幕。Size layer 寬度只依實際欄位總寬計算；大量資料使垂直捲動軸出現時，若欄位仍放得下，不得產生多餘的橫向捲動軸。 |
...
| 排序與篩選 | 提供欄位排序、Quick Search、Search Row 與 Excel-like 值篩選；`filterMode` 以 `false` 或模式陣列控制，陣列第一項是預設及目前模式，例如 `['excel', 'searchRow']` 預設顯示 Excel-like、`['searchRow', 'excel']` 預設顯示 Search Row。正規化後 `array.length > 1` 時 Header 右鍵功能表才提供篩選方式切換；單一項目只開放該模式。Header 排序符號下緣與欄位文字下緣對齊，窄欄位同時顯示 sort 與 filter icon 時兩者的可視間距為 `2px`；文字與數字欄位在空間不足時都可延伸並重疊顯示在 icon 下方。右對齊數字 Header 採三段式間距：欄寬足以容納完整標題與 filter icon 時保留 `24px` icon 空間且不得重疊；稍窄但完整標題仍放得下時保留與左對齊文字相同的 `6px` 外側間距並允許重疊；再窄時才取消間距。右對齊 Header 的 sort icon 使用絕對定位，使文字位置在切換排序前後保持一致。Default 與淺色主題的運算符 filter icon 使用一般字重的 `blue`，`black`／`dark-hive` 深色主題使用白色。`remote: true` 套用 Excel-like 值篩選後重新開啟同一欄位，仍列出套用前的完整候選值並保留目前勾選狀態。初始化 `filterRules` 只在 `filterMode` 包含 `'searchRow'` 時處理，並在第一次本機 view／遠端 request 前套用；Grid 建立後可用 `setFilterRules(rules)` 一次取代規則並同步 Search Row input，並用 `getFilterRules()` 取得包含最新 input 值的實際規則副本。有效且對應欄位的規則會將 Search Row 設為目前模式、填入搜尋 input 與 operator 符號；`remote: true` 不驗證或轉換自訂 `op`，遠端 POST 會將原規則 JSON 字串放入 Form Data 的 `filterRules` 欄位，未對應顯示 column 的伺服器規則也會保留。啟用 Search Row 初始化後會聚焦第一個搜尋輸入框。Search Row 顯示時，搜尋 input 按向下鍵會先把焦點轉到目前列同一 column 的 active cell，不移動 selected row；若目前 active cell／selected row 已在畫面外，會先捲動使其出現。再次按方向鍵才由 Grid 開始移動。active cell 位於第一列時按向上鍵，焦點會回到同一 column 的搜尋 input。 |
...
| 顯示自訂 | 提供 formatter、`formatItem`、`cellTemplate`、依 binding 設定 Header cell style、Row／GroupRow、`hitTest()` 與完整事件 API；所有公開事件均可用 constructor option 的 `(grid, eventArgs)` callback、Wijmo-compatible event object 或 `on()` 註冊。 |
...
欄位顯示切換使用 `grid.setColumnVisible(column, visible)`；`column` 可傳入 `grid.columns` 的 index，或該 Grid 的 column object。依 `binding` 操作時，先以 `grid.getColumn(binding)` 取得 column object，再傳給 `setColumnVisible()`；binding 字串不可直接作為 `setColumnVisible()` 的第一個參數。不要只修改 `grid.columns[index].visible`，否則不會自動更新 layout、選取狀態與畫面。
...
Search Row input 使用 `searchDelay` 控制停止輸入後的 debounce 時間，預設為 `400ms`；設為 `0` 時立即套用。本機與 `remote: true` 共用此設定。

Search Row input 的 `Enter`／`Tab` 會移到下一個可見欄位的搜尋 input，`Shift+Enter`／`Shift+Tab` 會移到上一個；目標 input 已有內容時自動全選，空白時維持一般游標。四種按鍵都會攔截外層表單預設提交，但不得取消既有 debounce timer，也不得立即套用或送出遠端查詢。輸入內容仍依 `searchDelay` 套用。

`remote: true` 時，Search Row UI 內部仍使用標準 operator，但 `getFilterRules()` 與遠端 request 一律送出相容符號：`starts`、`contains`、`ends` 分別送出 `..%`、`%..%`、`%..`；`not-starts`、`not-contains`、`not-ends` 分別送出 `!..%`、`!%..%`、`!%..`；`gte`、`gt`、`lte`、`lt`、`ne`、`eq` 分別送出 `>=`、`>`、`<=`、`<`、`<>`、`=`。傳入既有相容符號時維持相同符號，沒有對應符號的自訂 `op` 仍原樣送出。

`remote: true` 的 `op: "in"` 不分大小寫，Search Row 顯示 `IN`，並將陣列值序列化為逗號分隔字串：`["ZU001", "AV001"]` 送出時為 `"value":"ZU001,AV001"`。這項規則同時套用於 Excel-like filter、`setFilterRules()`、`getFilterRules()` 與 GET／POST request，送出時仍保留使用者傳入的 `op` 大小寫。

`remote: true` 的 `filterRules` 若對應到 Grid column，即使 `value` 是空字串，也會初始化 Search Row 並顯示該 `op` 的相容符號；空白的 Search input 不會產生該欄位的篩選規則，也不會送到後端。使用者輸入內容後，已知 operator 會以上述相容符號送出，清空後立即停止篩選該欄位。
...
| FabUI Diagram | `dist/fabui.diagram.js`、`dist/fabui.diagram.min.js`、`dist/fabui.diagram.css`、`dist/fabui.diagram.min.css` | 獨立附加 `fabui.Diagram`；依賴並必須晚於 FabUI core 載入，不會併入 `fabui.*` |
...
| 主題 | `dist/theme/fabui.<theme>.css`、`dist/theme/fabui.<theme>.min.css` | Default 以外 18 組固定 selector 覆蓋 CSS 與必要圖片；必須最後載入 |
...
預設 `npm run build` 產生 FabUI core 四個主檔與 `dist/theme/`；`npm run build:gantt` 可在已有 core 時單獨重建 Gantt。`npm run build:scheduler` 獨立產生四個 `fabui.scheduler.*` 檔案，不會將 Scheduler 寫入 core 入口；`npm run build:lite` 獨立產生四個 FabUI Lite 精簡檔，`npm run build:lite -- min` 則只保留 `fabui.lite.min.js`、`fabui.lite.min.css` 與必要圖片。Lite 內容為 FabGrid、內建 TreeGrid、Pivot、Chart 與必要依賴，不包含 Button、EditBox 公開控件、Diagram、Window、Menu、Tabs 等其他控件。`npm run build:theme` 只重建 `dist/theme/` 的 18 組非 Default 一般／壓縮 CSS 與必要圖片，`npm run build:theme -- min` 只保留壓縮 CSS 與必要圖片，不觸碰其他 bundle。所有 build 指令只產生 browser global JavaScript、壓縮 JavaScript、CSS 與壓縮 CSS，不產生任何 `.esm.*` 檔案。Vue 2 與 FabGrid jQuery wrapper 暫緩，不再由預設 build 或 core smoke gate 編譯／驗證。EditBox jQuery wrapper 已移除，`dist/` 也不再產生獨立 `dist/editbox.*`。
...
若只要重新產生 `fabui.lite.*`，可執行 `npm run build:lite`；只需要壓縮版時執行 `npm run build:lite -- min`。這兩個指令都不會清除或重建完整的 `fabui.*`。
