...
- Build 命令固定依照以下範圍執行：
  - `build`／`build fabui`：執行 `npm run build`，只編譯 `fabui.*`，不包含 Diagram、`fabui.lite.*`、`fabui.gantt.*`、`fabui.scheduler.*` 與 `fabui.htmleditor.*`；`dist/fabui.*` 不得包含 Diagram 或 HtmlEditor JavaScript／CSS。
  ...
  - `build htmleditor`：執行 `npm run build:htmleditor`，只編譯獨立 HtmlEditor，產生 `dist/fabui.htmleditor.js`、`dist/fabui.htmleditor.min.js`、`dist/fabui.htmleditor.css` 與 `dist/fabui.htmleditor.min.css`，不得重新編譯或寫入 `dist/fabui.*`。
  ...
  - `build all`：依序執行 `npm run build`、`npm run build:lite`、`build diagram`、`npm run build:gantt`、`npm run build:scheduler`、`npm run build:htmleditor`，編譯以上全部發佈輸出；一般 `build` 已包含 theme，因此不重複執行 `build theme`，也不得額外執行 `build clear`。
  - 可用半形逗號一次指定多個編譯範圍，格式固定為 `build <scope>,<scope> [min]`，逗號左右不得有空白；例如 `build fabui,htmleditor min` 必須依序編譯 FabUI core 與 HtmlEditor，並對兩個範圍都套用 `min`。可組合的 scope 為 `fabui`、`lite`、`diagram`、`gantt`、`scheduler`、`htmleditor`、`theme`，依使用者列出的順序各執行一次；重複 scope 只執行第一次。`all` 與 `clear` 必須單獨使用，不得放入逗號清單，也不得和其他 scope 組合。
  - 所有編譯命令都可在尾端加上 `min`，包含 `build min`、`build lite min`、`build diagram min`、`build gantt min`、`build scheduler min`、`build htmleditor min`、`build theme min` 與 `build all min`。加上 `min` 時，只產生並保留該範圍內檔名含 `.min.` 的壓縮版 JS／CSS，不得產生或保留同範圍的非壓縮輸出；`build htmleditor min` 只產生並保留 `dist/fabui.htmleditor.min.js` 與 `dist/fabui.htmleditor.min.css`；theme 所需圖片依賴仍須保留。`build clear` 不適用 `min`。
- 所有 build 命令一律不得產生 `.esm.js`、`.esm.min.js` 或其他 ESM 發佈檔；只輸出 browser global JavaScript、壓縮 JavaScript、CSS、壓縮 CSS 與必要 theme／圖片資產。
- Diagram、Gantt、Scheduler 與 HtmlEditor 必須保持獨立 bundle，不得併入 `fabui.*` core。Vue 2 與 FabGrid jQuery wrapper 目前暫緩，不得納入上述 build 命令。既有 Vue 2 與 FabGrid jQuery wrapper 原始碼及獨立 build script 保留，只有使用者未來明確要求恢復或單獨編譯 wrapper 時才可使用。FabUI EditBox jQuery wrapper 已移除，不得重新加入預設或獨立 build。
...
- 開發測試優先使用 source-mode `demo/dev-grid.html`，直接引用公開入口 `src/fabui.js` 與 `src/fabui.css`；build-mode 主 Demo 固定為 `demo/grid.html`。兩者的功能、控制項、預設值與互動必須保持一致，只允許 source／dist 載入方式及必要的 locale 載入差異。內部模組仍放在各自目錄。修改 source 或共用 Demo script 後要同步更新兩頁的 query version，避免瀏覽器快取造成誤判。
...
- FabGrid `allowMultiSorting` 預設為 `true`；設為 `false` 時，Header 的 `Shift + 點擊` 與程式呼叫 `toggleSort(colIndex, true)` 都必須退回單欄排序，只保留目前操作欄位，`sortingColumn`／`sortedColumn` 的 `e.multiSort` 也必須為 `false`。此規則同時適用本機與遠端排序。
...
- FabGrid `showFullscreenMenu` 預設為 `false`；只有設為 `true` 時，Header cell 右鍵選單才顯示 Grid 全螢幕／離開全螢幕項目。此設定不得影響 `toggleFullscreen()` API，清除篩選、列號與匯出等其他選單項目不受影響。
- FabGrid `unselectRow(row?)` 必須同時支援單選與多選。單選時只能取消目前 selected row，成功後 `selectedRows`／`selectedItems` 為空、列選取背景消失，且排序、篩選與 refresh 後保持未選取；active cell 不屬於此 API 的行為契約。省略 `row` 時使用目前 selected row；無 selected row 或指定其他列時回傳 `false`。
...
- FabGrid 本機與 TreeGrid 多欄排序必須先為每列準備各欄排序值，再進入 comparator；不得在 comparator 的每次比較中重複讀取 binding、轉換 Date、number 或 lowercase 字串。排序結果、null 順序與既有 data type 語意必須保持相容。
- FabGrid scroll render 必須重用已完成的 layout；只有水平可視欄位 range 改變時才重建 Header／Footer，純垂直捲動不得重建 Header、Footer 或 Pager。Row Header、selection 與 body virtualization 仍須跟隨目前 row range 更新，不得因最佳化留下舊 cell。
- TreeGrid 收合狀態必須讓既有 collapsed item 陣列與 object-item `WeakSet` 索引保持同步；單筆展開／收合、指定層級收合、全部展開與重建 tree state 後都必須同步更新，不得只更新其中一邊。
...
- `fabui.HtmlEditor` 是獨立附加元件，原始碼位於 `src/htmleditor/`，source 入口為 `src/fabui.htmleditor.js`／`src/fabui.htmleditor.css`，輸出固定為 `dist/fabui.htmleditor.js`、`.min.js`、`.css`、`.min.css`。FabUI core 不得匯入或公開 HtmlEditor；Browser global 必須先載入 `fabui.*` 再載入 `fabui.htmleditor.*`，獨立 bundle 只附加 `fabui.HtmlEditor`。HtmlEditor 使用 pure JavaScript，畫面與預設工具列規格參考 Summernote，支援文字／段落／字型、獨立文字顏色與文字底色色票、清單、表格、連結、Data URL／外部網址圖片、影片、水平線、undo／redo、全螢幕、HTML 原始碼、拖曳高度、自訂工具列、textarea 表單同步、三語系、19 組 theme、Control registry 與 dispose；不得依賴 Summernote、jQuery 或 Bootstrap。預設 `color` group 依序提供 `color` 與 `backcolor` action，分別執行 `foreColor` 與 `backColor`。工具列與對話框必須重用既有 `fabui.Button`、`fabui.Window`、`fabui.EditBox`、`fabui.CheckBox` 與 `fabui.FileBox`。Popup document／window listener 只在開啟期間綁定；resize pointer listener 只在拖曳期間綁定，關閉、pointer up、pointer cancel 或 dispose 時立即解除。
...
- `selectionMode` 支援 `Cell` 與連續矩形 `CellRange`；後者支援滑鼠拖曳、列號點擊／拖曳選取所有可見欄位、`Shift + Click`、`Shift + 方向鍵`與 TSV clipboard copy。CellRange 使用資料列選取背景，active cell 保留一般 cell 背景及與單格選取相同的完整四邊框，範圍四周外框必須共用 `activeCellBorder` 粗細；設為 `0` 時不顯示外框，且不得因 `highlightActiveRow` 額外填滿 active row。Default 與其他 18 組主題都必須明確定義 `--fg-range-selected-bg`、`--fg-range-active-bg`、`--fg-range-border`，分別沿用該主題的列選取背景、cell 背景與選取邊框色。CellRange 雙擊必須由同一資料格的連續 pointer 操作成立，`pointercancel` 不得觸發雙擊，完成 pointer 選取後不得再由 click 重複套用 selection 或 render。`highlightActiveRow` 預設為 `true`，只控制 `Cell` 模式的 active row 背景，不得隱含改變多選列；active cell 邊框預設為 1px，`setRowHeaderWidth(width)` 可在 runtime 調整列號欄寬並自動 refresh。
...
13. Pagination
   ...
   - `remote: true` 支援 `url + method` 內建請求與自訂 Promise loader；method 限定為 GET 或 POST。內建請求的 `credentials` 預設為 `'same-origin'`，另支援 `'include'` 與 `'omit'`；`'include'` 等同 XHR `withCredentials: true`。自訂 loader 必須自行設定 Fetch credentials。
...
- 每次使用者明確要求 build 時，依「開發工作流程」定義的 `build`／`build fabui`、`build lite`、`build diagram`、`build gantt`、`build scheduler`、`build htmleditor`、`build theme`、`build clear`、`build all`、逗號分隔多範圍或上述命令的 `min` 形式執行對應操作與檢查；逗號左右不得有空白，`min` 必須套用到清單中的每個範圍。Vue 2 與 FabGrid jQuery wrapper 暫緩且不納入 build gate。
