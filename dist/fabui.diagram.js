/*! FabUI Diagram browser global | Requires FabUI core */
(function(global) {
'use strict';
...
function diagramEventTargetsInteractiveControl(event, boundary) {
...
function normalizeDiagramToolboxOrder(value, fallback) {
  var result = [];
...
function reorderDiagramToolboxOrder(
  order,
  sourceCategory,
  targetCategory,
  after
...
function constrainDiagramFloatingToolbox(
  left,
  top,
  width,
  height,
...
function constrainDiagramViewToolbar(
  left,
  top,
  width,
  height,
...
function diagramViewportCanPan(
  scrollWidth,
  scrollHeight,
  clientWidth,
  clientHeight
...
function diagramToolboxShouldDock(clientX, workspaceLeft, threshold) {
...
function diagramPropertiesShouldDock(
  clientX,
  workspaceLeft,
  workspaceWidth,
  threshold
...
function normalizeDiagramDockSide(value, fallback) {
...
function normalizeDiagramSameSideDockMode(value) {
  return String(value || '').toLowerCase() === 'stacked' ?
    'stacked' :
    'tabs';
}

function getDiagramPanelDockSide(
  clientX,
  workspaceLeft,
  workspaceWidth,
  threshold
...
function normalizeDiagramTheme(value) {
...
function normalizeDiagramLocale(value) {
...
function normalizeDiagramHyperlink(value) {
...
function normalizeDiagramHyperlinkTrigger(value) {
  return String(value || '').toLowerCase() === 'dblclick' ?
    'dblclick' :
    'click';
}

function diagramNodeHyperlinkMatchesTrigger(node, trigger) {
...
function getDiagramShapeBoundaryPoint(node, target) {
...
function findBestDiagramConnectionPoints(
  fromNode,
  toNode,
  fromPoint,
  toPoint
...
function calculateDiagramMoveAlignment(
  nodes,
  connectors,
  movingNodeIds,
  threshold
...
function getDiagramConnectionPoint(node, name) {
...
function findDiagramNodesInRect(nodes, rect) {
...
function calculateDiagramConnectorPath(
  fromNode,
  toNode,
  type,
  fromPoint,
...
function calculateDiagramNodeResize(node, direction, dx, dy, minWidth, minHeight) {
  var result = diagramAssign({}, node);
...
function calculateDiagramViewportOffset(
  viewportRect,
  svgRect,
  scrollLeft,
  scrollTop,
...
function calculateDiagramAnchoredScroll(
  scrollOffset,
  anchorOffset,
  contentOffsetBefore,
  oldZoom,
...
function diagramFullscreenSizeChanged(previous, next) {
