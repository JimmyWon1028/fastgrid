/*! FabUI Lite browser global | FabGrid, TreeGrid, Pivot and Chart */
(function(global) {
var FABUI_CONFIG = {
  request: {
...
function normalizeChartLocale(value) {
...
function normalizeChartTheme(value) {
...
function prepareSortValue(value, type) {
  if (value == null) {
    return null;
  }
  if (type === 'number') {
...
function comparePreparedValues(a, b) {
  if (a == null && b == null) {
    return 0;
  }
  if (a == null) {
...
function normalizeRemoteCredentials(value) {
  var credentials = String(value || '').toLowerCase();
  return credentials === 'include' || credentials === 'omit' ? credentials : 'same-origin';
}

function createRemoteRequest(url, method, params, credentials) {
...
function getSizeLayerWidth(fixedLeftWidth, frozenWidth, scrollableWidth, frozenRightWidth) {
...
function canSwitchFilterMode(options) {
...
function resolveFabGridRequestCredentials(credentials, getGlobalConfig) {
  var config;
  if (credentials !== undefined) {
    return normalizeRemoteCredentials(credentials);
  }
  config = typeof getGlobalConfig === 'function' ? getGlobalConfig() : null;
  return normalizeRemoteCredentials(config && config.request ? config.request.credentials : null);
}

function createFabGridFactory(editorDefinitions, getGlobalConfig) {
  'use strict';
  ...
  function FabGrid(element, options) {
    ...
    this.options = mergeOptions(DEFAULT_OPTIONS, suppliedOptions);
    this.options.credentials = resolveFabGridRequestCredentials(
      suppliedOptions.credentials,
      getGlobalConfig
    );
    ...
    if (this.options.remote === true) {
      ...
      this.focusInitialHeaderSearchInput();
    }
  }
  ...
  FabGrid.prototype.createDom = function() {
    ...
    this._pagerElement = root.querySelector('.fg-pager');
    this._paginationElement = root.querySelector('.fg-pagination');
    this.pager = this._pagerElement;
    this.pagination = this._paginationElement;
  ...
  FabGrid.prototype.applyLocaleToDom = function() {
    ...
    if (this._paginationElement) {
      this._paginationElement.setAttribute('aria-label', this.getText('pagination.ariaLabel'));
    }
  };
  ...
  FabGrid.prototype.bindDomEvents = function() {
    ...
    this._paginationElement.addEventListener('click', this._boundPaginationClick);
    this._paginationElement.addEventListener('change', this._boundPaginationChange);
  ...
  FabGrid.prototype.unbindResizeObserver = function() {
  ...
  FabGrid.prototype.getPager = function() {
    return this._pagerElement || this.pager;
  };
  ...
  FabGrid.prototype.dispose = function() {
    ...
    this._paginationElement.removeEventListener('click', this._boundPaginationClick);
    this._paginationElement.removeEventListener('change', this._boundPaginationChange);
    ...
    Object.defineProperties(FabGridCtor.prototype, {
      ...
      selectedItems: {
        get: function() {
          ...
          item = this.rowSelection == null ? null : this.view[this.rowSelection];
          return item ? [item] : [];
        }
      },
      selectedRows: {
        get: function() {
          ...
          return row != null && item ? [rowCollection[row]] : [];
        }
      },
      selectedRanges: {
        get: function() {
...
function getPivotSlicerValues(engine, field) {
  var values = [];
...
function createPivotSlicerFactory(
  Control,
  registerControl,
  unregisterControl,
  PivotEngine,
...
function normalizePivotWorkspaceTheme(value) {
  var theme = String(value == null ? '' : value).trim().toLowerCase();
  if (theme === 'pepper') theme = 'pepper-grinder';
