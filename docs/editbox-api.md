...
`fabui.EditBox` 是 FabUI core 內建的 pure JavaScript 編輯控件，以單一 class 取代原本分散的 TextBox、NumberBox、DateBox、ComboBox 公開入口。`text`、`number`、`date`、`combo`、`color` 五種模式共用 TextBox 視覺基準與同一組 FabGrid editor definitions。
...
| Option | 預設值 | 說明 |
| --- | ---: | --- |
| `editor` | 自動判斷 | `text`、`number`、`date`、`combo`、`color` 或相容別名。 |
...
| `iconWidth` | `18` | 一般 icon 寬度；date、combo、color 預設使用 `28`。 |
...
| `theme` | `inherit` | 預設從外層 `fg-theme-*` class 繼承，找不到時使用 `default`；也可指定 `bootstrap`、`cupertino`、`material`、`material-blue`、`material-teal`、`metro` 系列、`sunny`、`pepper-grinder`、`dark-hive`、`black`。 |
...
| `panelWidth` / `panelHeight` | `null` / `300` | Popup 尺寸；寬度預設跟隨 EditBox。 |
...
- `fabui.EditBox.editorTypes`：目前公開的五種 editor 類型。
