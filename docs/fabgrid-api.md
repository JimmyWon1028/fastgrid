...
| `filterMode` | `false \| Array<'excel' \| 'searchRow'>` | `['excel', 'searchRow']` | 控制可用的欄位篩選模式，陣列第一項為預設及目前模式。`['excel', 'searchRow']` 預設顯示 Excel-like；`['searchRow', 'excel']` 預設顯示 Search Row。正規化後 `array.length > 1` 時，Header 右鍵功能表才提供篩選方式切換；單一項目只開放該模式且不顯示切換項目。`false`、`[]` 或全部為無效值時關閉兩種欄位篩選。重複值會移除、未知值會忽略。切換目前模式時會先清除另一套欄位條件，右下角 Quick Search 保留。 |
| `filterRules` | `Array<{field, op, value}> \| string` | `[]` | 初始化 Search Row 篩選；可傳陣列或相同內容的 JSON 字串。只有 `filterMode` 包含 `'searchRow'` 時才會處理；有效且對應 Grid column 的規則會將 `'searchRow'` 移到第一項、填入對應欄位 input 並顯示 operator 符號。本機第一次 view 與遠端第一次 POST Form Data／GET query 都直接套用。`remote: true` 時不驗證或轉換 `op`，會原樣送給後端；沒有對應 Grid column 的規則仍會送出。空值與一般陣列值會忽略；但 `remote: true` 的 `op: "in"` 陣列值會轉成逗號分隔字串。 |
...
「清除篩選」會清除 predicate、全域搜尋與所有欄位搜尋；「列號」的下層功能表提供關閉、顯示列號及只顯示 cell，並以勾選標示目前模式。功能表文字跟隨目前 locale，篩選模式與 fullscreen 項目會依當下狀態切換文字。TreeGrid 樹欄資料 cell 是唯一例外，右鍵時使用同一 popup 顯示全部展開／全部疊合。
